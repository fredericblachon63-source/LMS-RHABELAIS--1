...
export default function AdminFormations() {
  ...
  const handleEdit = (f: Formation) => {
    setForm({ numero: f.numero, titre: f.titre, description: f.description || '', slide_url: f.slide_url || '' })
    setEditingId(f.id)
    setShowForm(true)
  }
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">URL du PDF (Supabase Storage)</label>
              ...
              <div key={f.id} className="bg-white rounded-xl shadow-sm p-6 border">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleEdit(f)} className="text-gray-400 hover:text-blue-600 text-sm">Modifier</button>
                    <button onClick={() => handleDelete(f.id)} className="text-gray-400 hover:text-red-600 text-sm">Suppr</button>
                  </div>
                </div>
                <h3 className="font-semibold text-gray-900 mb-2">{f.titre}</h3>
                {f.description && <p className="text-gray-500 text-sm">{f.description}</p>}
                {f.slide_url && <p className="text-blue-600 text-xs mt-2 truncate">PDF: {f.slide_url}</p>}
              </div>
            ))}
          </div>
        )}
